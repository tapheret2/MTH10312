...
HAVING SUM(c.so_luong) > (
    SELECT AVG(s.tong_so_luong)
    FROM SAN_PHAM AS s
    JOIN DANH_MUC AS dm ON s.ma_danh_muc=dm.ma_danh_muc
...
WITH GiaTriTheoDanhMuc AS (
    SELECT dm.ma_danh_muc,dm.ten_danh_muc,SUM(c.so_luong * c.don_gia_nhap) AS tong_gia_tri
    FROM DANH_MUC AS dm
    JOIN SAN_PHAM AS s ON dm.ma_danh_muc=s.ma_danh_muc
    JOIN CHI_TIET_DON_NHAP AS c ON s.ma_san_pham=c.ma_san_pham
    JOIN DON_NHAP_HANG AS d ON c.ma_don_nhap=d.ma_don_nhap
    WHERE d.trang_thai IN ('R','r') AND d.ngay_dat_hang BETWEEN '2025-05-01' AND '2025-05-31'
    GROUP BY dm.ma_danh_muc,dm.ten_danh_muc
)
SELECT ma_danh_muc,ten_danh_muc,tong_gia_tri
